...
class RTC {
  ...
  viceVersa(rootA: string, room: string) {
    wss.to(rootA).emit("vice:versa", "from$rootB" + room);
  }

  forcefulleaveRoom(room: string) {
...
const detectIp = async (Req: Request) => {
...
wss.on("connection", (websocket: Socket) => {
  ...
  websocket.on("pause:stream", async (room: string) => {
    sendSync(
      room,
      {
        intension: true,
  ...
  websocket.on("i:am:done", async (room: string) => {
